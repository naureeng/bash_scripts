...
experimentName  = '180712Merfish_3D'; % provide experiment name, no spaces!

% set directories
dataDir     = '//nfs//winstor//isogai//smfish_data//disk2//Mat//'; % specify directory of data, no spaces!
thisExpDir  = fullfile(dataDir,experimentName);
addpath('//nfs//nhome//live//naureeng//saveastiff_4.4'); 
...
%                     
                            saveastiff(uint16(imageData.dapi), fullfile(thisResultDir,stackName,strcat(dapiName,'.tif')));
                            saveastiff(uint16(imageData.fid), fullfile(thisResultDir,stackName,strcat(fidName1,'.tif')));
                            saveastiff(uint16(imageData.fid), fullfile(thisResultDir,stackName,strcat(fidName2,'.tif')));
                            saveastiff(uint16(imageData.cy3), fullfile(thisResultDir,stackName,strcat(cy3Name,'.tif')));
                            saveastiff(uint16(imageData.cy5), fullfile(thisResultDir,stackName,strcat(cy5Name,'.tif')));
